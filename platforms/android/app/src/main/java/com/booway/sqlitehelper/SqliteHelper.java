...
public class SqliteHelper extends CordovaPlugin
{
    ...
    {
        final String createUserTable = "create table " + TABLE_NAME + " (" + C_ID
                + " integer primary key autoincrement, " + "" + C_USERNAME + " text," + C_BIRTHDAY + " text)";
        if (writableDatabase != null)
        {
    ...
    public void insertInfo(JSONArray args)
    { //插入记录
        if (writableDatabase == null)
        {
            callback.error("请初始化数据库");
